...
SELECT 
    table_name, table_rows AS TABLE_ROWS
FROM
...
SELECT 
    COUNT(country) AS Number_movies_produced
FROM
    movie
WHERE
    (country LIKE '%USA%'
...
-- Type your code below:
SELECT 
    genre, COUNT(genre) AS Number_of_movies
FROM
    genre
GROUP BY genre
...
SELECT count(*) AS Number_of_single_genre_movies FROM No_count;
...
ORDER  BY Count(production_company) DESC
LIMIT 2; 
...
SELECT 
    languages, SUM(total_votes) AS total_number_of_votes
FROM
    ratings AS r
        INNER JOIN
    movie AS m ON r.movie_id = m.id
...
GROUP BY languages LIKE '%german%' , languages LIKE '%italian%'
...
FROM
    ratings AS r
        INNER JOIN
    role_mapping AS ro ON r.movie_id = ro.movie_id
        INNER JOIN
    names AS n ON n.id = ro.name_id
WHERE
    median_rating >= 8
        AND category = 'actor'
GROUP BY name
...
FROM
	...
	role_mapping AS a
        ...
        movie AS m
            ...
            ratings AS r
				ON m.id=r.movie_id
WHERE category = 'actress' AND LOWER(languages) like '%hindi%' and country like "%INDIA%"
...
SELECT 
    genre,
    COUNT(r.movie_id) AS no_of_movies,
    CASE
...
AS
  ...
  FROM     duration_summary window w1 AS (ORDER BY avg_duration rows unbounded preceding),
     ...
     AS (WITH summary
              AS (SELECT g.genre,
                         ...
                         CASE
                           ...
                           Round(
                           Substring(m.worlwide_gross_income, 4) / 73) )
...
SELECT     production_company,
           Count(m.id)                           AS movie_count,
           ROW_NUMBER()OVER(ORDER BY count(production_company) DESC) AS prod_comp_rank
FROM       ratings                                             AS r
INNER JOIN movie                                               AS m
ON         r.movie_id=m.id
...
(
	...
	(		
		...
		JOIN movie m 
		ON m.id = r.movie_id
		ORDER BY n.name, date_published
	)
...
SELECT name_id AS director_ID,
	   ...
	   ROUND(AVG(days_diff))AS avg_inter_movie_days,
